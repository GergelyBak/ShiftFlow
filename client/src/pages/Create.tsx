...
const DesiredForm = ({ isAdmin }: { isAdmin: boolean }) => {
  ...
  const [selectedType, setSelectedType] = useState<'available' | 'unavailable'>('available');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setAvailability((prev) => {
        const filtered = prev.filter((a) => new Date(a.date).toDateString() !== selectedDay.toDateString());
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      setAvailability((prev) =>
        prev.filter((a) => new Date(a.date).toDateString() !== selectedDay!.toDateString())
      );
  ...
  const handleShiftSubmit = async () => {
    if (!shiftDate || !shiftStart || !shiftEnd) return toast.error(t('fillAllFields'));
    ...
    try {
      ...
      addNotification(`Shift created for ${shiftDate} (${shiftStart}-${shiftEnd})`);
    ...
    <div className='space-y-4'>
      {isAdmin && (
        <div className='bg-slate-200/60 dark:bg-slate-800 rounded-3xl p-1.5'>
          <div className='bg-white dark:bg-slate-900 rounded-2xl px-4 py-4 space-y-3'>
            ...
            <button onClick={handleShiftSubmit} disabled={shiftLoading}
      ...
      <div className='bg-slate-200/60 dark:bg-slate-800 rounded-3xl p-3'>
        <div className='flex items-center justify-between mb-3'>
          ...
          <div className='flex items-center gap-1 bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded-full'>
            <button onClick={() => { const d = new Date(currentDate); d.setDate(d.getDate() - 7); setCurrentDate(d); }}
              className='text-slate-600 dark:text-slate-300 p-0.5'><ChevronLeft size={14} /></button>
            <span className='text-xs font-medium text-slate-700 dark:text-slate-200 px-1'>{weekLabel}</span>
            <button onClick={() => { const d = new Date(currentDate); d.setDate(d.getDate() + 7); setCurrentDate(d); }}
              className='text-slate-600 dark:text-slate-300 p-0.5'><ChevronRight size={14} /></button>
          </div>
        </div>
        ...
        <div className='flex items-center gap-3 mb-3 px-1'>
          ...
          <span className='flex items-center gap-1 text-xs text-slate-400'>
            <span className='w-3 h-3 rounded-full bg-red-400 inline-block' /> Unavailable
          </span>
        </div>
        ...
        <div className='grid grid-cols-7 gap-1.5'>
          {weekDays.map((day, i) => {
            ...
            const textColor = avail ? 'text-white'
            ...
            return (
              <button key={i} onClick={() => !isPast && handleDayClick(day)} disabled={isPast}
                ...
                <span className={`text-base font-bold ${textColor}`}>{day.getDate()}</span>
                {avail && (
                  <span className='text-white/80 text-[9px] leading-none'>{avail.startTime}</span>
                )}
              </button>
            );
          })}
      ...
      {selectedDay && (
        <div className='fixed inset-0 bg-black/40 z-50 flex items-end justify-center' onClick={() => setSelectedDay(null)}>
          <div className='bg-white dark:bg-slate-900 w-full max-w-md rounded-t-3xl p-6 pb-10' onClick={(e) => e.stopPropagation()}>
            <div className='flex items-center justify-between mb-4'>
              <div>
                ...
                <p className='text-xl font-bold text-slate-900 dark:text-white'>
                  {selectedDay.toLocaleDateString(locale, { day: '2-digit', month: 'long', year: 'numeric' })}
                </p>
              </div>
              <button onClick={() => setSelectedDay(null)} className='bg-slate-100 dark:bg-slate-800 p-2 rounded-full'>
            ...
            <div className='bg-slate-100 dark:bg-slate-800 rounded-2xl p-1 flex mb-4'>
              ...
              <button onClick={() => setSelectedType('unavailable')}
...
const AbsenceForm = () => {
  ...
  useEffect(() => { fetchMyAbsences(); }, []);

  const fetchMyAbsences = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await api.get('/absences/my', { headers: { Authorization: `Bearer ${token}` } });
      setMyAbsences(res.data);
    } catch { console.error('Failed to load absences'); }
  };

  const handleSubmit = async () => {
    if (!startDate || !endDate || !reason) return toast.error(t('fillAllFields'));
    if (new Date(endDate) < new Date(startDate)) return toast.error('End date must be after start date');
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const res = await api.post('/absences', { startDate, endDate, reason }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const statusColor = (status: string) => {
    if (status === 'approved') return 'text-green-500 bg-green-500/10 border-green-500/30';
    ...
    <div className='space-y-4'>
      <div className='bg-slate-200/60 dark:bg-slate-800 rounded-3xl p-1.5'>
        <div className='bg-white dark:bg-slate-900 rounded-2xl px-4 py-5 space-y-4'>
          ...
          <div>
            <label className='text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block'>{t('reason')}</label>
            <input type='text' placeholder='e.g. Urlaub, Krankheit...' value={reason} onChange={(e) => setReason(e.target.value)}
      ...
      {myAbsences.length > 0 && (
        <div className='bg-slate-200/60 dark:bg-slate-800 rounded-3xl p-1.5'>
          <p className='text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide px-3 pt-2 pb-1'>My Requests</p>
          <div className='space-y-1'>
            {myAbsences.map((a) => (
              <div key={a._id} className='bg-white dark:bg-slate-900 rounded-2xl px-4 py-3 flex items-center justify-between'>
                <div>
                  <p className='text-sm font-medium text-slate-800 dark:text-white'>
                    {formatDate(a.startDate)}{a.startDate !== a.endDate && ` - ${formatDate(a.endDate)}`}
                  </p>
                  <p className='text-xs text-slate-400 mt-0.5'>{a.reason}</p>
                </div>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${statusColor(a.status)}`}>
                  {a.status}
                </span>
              </div>
            ))}
